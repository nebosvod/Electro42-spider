...
namespace spider_electro42
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            result = MySqlLib.MySqlData.MySqlExecute.SqlScalar("SELECT electro42_datetime FROM resources.`electro42` ORDER BY electro42_datetime DESC LIMIT 0,1", conn_str);
            string date_from_mysql = result.ResultText;

            string[] portnames = SerialPort.GetPortNames();
            SerialPort port = new SerialPort("COM9", 9600, Parity.None, 8, StopBits.One);
            byte[] buff = { 0x42, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x5F, 0x22 };
            //byte[] buff = { 0x42, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01 };
            byte[] buff2 = { 0x42, 0x08, 0x13, 0x97, 0xD9 };
            byte[] req = { };
            ...
            for (k = Convert.ToInt32(Math.Floor(time_interval.TotalMinutes / 30)); k >= 1; k--)
            {
                ...
                byte[] data111 = { 0x42, 0x06, 0x03, result_mem[2], result_mem[3], 0x0F };
                req = GetCRC(data111);
                ...
                result = MySqlLib.MySqlData.MySqlExecute.SqlNoneQuery("INSERT INTO resources.`electro42` (`electro42_datetime`,`electro42_active`,`electro42_reactive`) VALUES (" + "'" + date_prof + time_prof + "'" + "," + "'" + active_prof_str + "'" + "," + "'" + reactive_prof_str + "'" + ")", conn_str);
